...
export function ProductionChartWidget({
  ...
  // Calculate completion rate for each item
  aggregatedData.forEach(item => {
    item.completionRate = item.planned > 0 ? (item.actual / item.planned) * 100 : 0;
  });

  const getBarColor = (entry: any) => {
    if (entry.completionRate >= 100) return 'hsl(var(--success))';
    if (entry.completionRate >= 80) return 'hsl(var(--warning))';
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    const completionRate = data.planned > 0 ? ((data.actual / data.planned) * 100).toFixed(1) : 0;
    
    return (
      <div className="bg-popover border rounded-lg p-3 shadow-lg">
        <p className="font-medium mb-2">{data.dayName}, {data.dateFormatted}</p>
        <div className="space-y-1 text-sm">
          <p className="text-blue-500">Planejado: {data.planned.toLocaleString('pt-BR')}</p>
          <p className="text-green-500">Realizado: {data.actual.toLocaleString('pt-BR')}</p>
